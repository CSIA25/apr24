...
// ADD 'citizen' to the UserRole type
type UserRole = 'volunteer' | 'ngo' | 'superadmin' | 'restaurant' | 'citizen' | null;
      ...
      if (authUser) {
        try {
          ...
          if (docSnap.exists()) {
            ...
            // ADD 'citizen' to the valid roles check
            const validRoles: UserRole[] = ['volunteer', 'ngo', 'superadmin', 'restaurant', 'citizen'];
            const combinedUser: AuthUser = {
              ...authUser,
              // Ensure the role fits the UserRole type, default to null if undefined/invalid
              role: validRoles.includes(userData.role as UserRole)
                ? userData.role
                : null,
